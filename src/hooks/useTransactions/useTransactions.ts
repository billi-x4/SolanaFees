...
const aggregateTransactions = (
	...
	return {
		aggregation,
		categorizations,
		fees: { total: aggregation.feesTotal, breakdown: {} }
	}
}
	...
	useEffect(() => {
		;(async () => {
			...
			if (wallets.current.includes(fullAddress)) {
				setStatus((prev) => ({
					...
					summary: {
						...
						fees: prev.summary?.fees || null, // Ensure 'fees' is always present as required by WalletResult
					},
					state: "done",
			...
			setTimeout(async () => {
				...
				const aggregatedResult = aggregateTransactions(
					fullAddress,
					transactions,
					cachedSummary.current
				);
